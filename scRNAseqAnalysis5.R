...
m.immune <- readRDS(file=paste(filepath,'outputs/m_immune_final_2.RDS',sep=""))
m.tumor <- readRDS(file=paste(filepath,'outputs/m_tumor_final_2.RDS',sep=""))

# ```
# ### RE-SUBSET CELLS ###
# ```{r }
`%ni%` <- Negate(`%in%`)
immune1 <- subset(m.immune, subset=seurat_clusters  %ni% c(8)) # everything except these clusters from prev. clustered immune cells
immune2 <- subset(m.tumor, subset=seurat_clusters %in% c(7)) # these clusters from prev. clustered tumor cells
#immune <- merge(immune1, immune2)

tumor1 <- subset(m.immune, subset=seurat_clusters  %in% c(8)) # these clusters from prev. clustered immune cells
tumor2 <- subset(m.tumor, subset=seurat_clusters %ni% c(7)) # everything except these clusters from prev. clustered tumor cells
...
pdf(file=paste(filepath,'outputs/cluster_plots_5.pdf',sep=""), width=15, height=9)
...
saveRDS(m.immune, file=paste(filepath,'outputs/m_immune_final_3.RDS',sep=""))
saveRDS(m.tumor, file=paste(filepath,'outputs/m_tumor_final_3.RDS',sep=""))
